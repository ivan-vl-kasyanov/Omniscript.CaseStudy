...
namespace Omniscript.CaseStudy.Server.DataAccess.Clients
{
    ...
    public sealed class QueueClient : IDisposable
    {
        ...
        public QueueClient()
        {
            ...
            DisposeClientToServerQueue();
            DisposeClientFromServerQueue();
            try
            {
                Channel.QueueDeclare(
                    RabbitMqClientToServerQueueName,
                    false,
                    false,
                    false);
                Channel.QueueDeclare(
                    RabbitMqClientFromServerQueueName,
                    false,
                    false,
                    false);
            }
            ...
            {
                var exceptionMessage = "Unable to initialize the queues.";
        ...
        private void DisposeClientToServerQueue()
        {
            if (Channel != null)
            {
                try
                {
                    Channel.QueueDeleteNoWait(RabbitMqClientToServerQueueName);
                }
                catch { }
            }
        }

        private void DisposeClientFromServerQueue()
        {
            if (Channel != null)
            {
                try
                {
                    Channel.QueueDeleteNoWait(RabbitMqClientFromServerQueueName);
                }
                catch { }
            }
        }
